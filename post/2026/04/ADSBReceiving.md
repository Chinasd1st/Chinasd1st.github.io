---
url: /post/2026/04/ADSBReceiving.md
---
...
`--interactive --net` 参数开启了交互模式，访问<http://localhost:8080/index.html>即可打开 Tar1090。
...
* **Hangzhou/Xiaoshan (ZSHC)**
  * **跑道与通信信息**：
    * `22 L 118`：
      * `22`：表示进近或离场程序对应的**跑道磁方位**约为 220°（即跑道 22 方向，对应实际跑道 07/25 的西端或相关程序）。
      * `L`：在此图中通常指该程序关联的**左侧跑道**或特定程序标识（萧山机场现运行两条平行跑道：南跑道 07/25、北跑道 06/24）。
      * `118`：对应**塔台管制频率 118.3 MHz**，即航空器在使用南塔台（South Tower）管制区域起降时，与机场塔台建立通信的主要频率。
...
$$
L\_{\frac{1}{4}\lambda} \approx \frac{275}{4} \text{mm} \approx 68.75 ,\text{mm}
$$
...
| 术语       | 解释                                                              | 主要工作频率                          | 内容                                                                                                                                                        | 接收                                                                                                                                            |
| ---------- | ----------------------------------------------------------------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **UAT**    | Universal Access Transceiver通用接入收发机                    | **978 MHz**                           | 美国特有的 ADS-B 另一种链路（主要用于小型通用航空飞机）。除了位置、速度等，还能同时广播 **FIS-B 天气和飞行信息**（免费图形天气）。在中国几乎没有使用。      | **可以**，但在中国基本收不到（信号极少或无）。需要单独的 978MHz 天线 + dump978 软件 + 另一根 RTL-SDR。效果在美国很明显。                        |
| **ADS-R**  | Automatic Dependent Surveillance - RebroadcastADS-B 重播服务  | 978 MHz 或 1090 MHz（地面站重播）     | 地面站把一种频率的 ADS-B 数据（例如 1090 的飞机）**转换重播**到另一种频率（978），让只装单频接收机的飞机也能看到另一频段的飞机。主要在美国使用。            | **可以接收**（如果附近有美国 FAA 地面站重播，会出现在 1090MHz 上）。在中国基本没有此服务，所以几乎收不到。                                      |
| **TIS-B**  | Traffic Information Service - Broadcast交通信息服务广播       | 978 MHz 或 1090 MHz（地面站广播）     | 地面雷达（二次雷达）探测到**没有 ADS-B 的飞机**（只装老式 Mode A/C/S 应答机），然后由地面站广播这些飞机的位置信息给装了 ADS-B In 的飞机，帮助补全画面。     | **可以接收**（如果地面站广播，会出现在你的 1090MHz 信号里）。在中国较少见，主要在美国 FAA 覆盖区常见。位置精度比真实 ADS-B 差一些，会轻微跳动。 |
| **Mode-S** | Mode Select选择模式应答机                                     | **1090 MHz**                          | 现代飞机应答机（Transponder）的标准模式。比老式 Mode A/C 先进，能提供唯一 24 位 ICAO 地址、呼号、更多数据，还支持 **ADS-B**（Extended Squitter）。          | **可以接收**。你现在的 dump1090 已经能收到 Mode-S 报文（很多飞机显示的 ICAO、Callsign 就是来自 Mode-S）。它是 ADS-B 的基础。                    |
| **ADS-C**  | Automatic Dependent Surveillance - Contract合同式自动相关监视 | **VHF 数据链** 或卫星链路（不是广播） | 飞机与**地面 ATC** 之间的点对点数据链路。飞机按“合同”（约定）周期性发送位置、速度、高度等给特定地面站，主要用于**海洋、偏远地区**监视（不是广播给所有人）。 | **普通人基本无法接收**。它是专用合同式链路，不是公开广播信号。用 RTL-SDR 收不到（需要特殊航空数据链设备和授权）。                               |
